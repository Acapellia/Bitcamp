...
namespace SecondWatch {
    public partial class Form1 : Form {
        ...
        private void Timer_Tick(object sender, EventArgs e) {
            dt = DateTime.Now;
            sec = dt.Second;
            secX = this.centX + (RADIUS * Math.Cos(6*(sec-15) * Math.PI / 180));
            secY = this.centY + (RADIUS * Math.Sin(6*(sec-15) * Math.PI / 180));
            //Console.WriteLine($"({(int)secX},{(int)secY})");
            Invalidate();
        }
        ...
        void DrawWatchNum(Graphics g, int cnt) {
            ...
            g.DrawLine(new Pen(Brushes.White,3), (float)centX, (float)centY, (float)secX, (float)secY);
        }
