...
select sname from student where substr(sname,1,1) = '공';
--3) 교수의 지위를 한글자로 검색하세요(ex. 조교수 -> 조)
select pname 교수이름, substr(orders,1,1) 직위 from professor;
--4) 일반 과목을 기초 과목으로 변경해서 모든 과목을 검색하세요
--   (ex. 일반화학 -> 기초화학)
select cname "과목 변경전", replace(cname,'일반','기초') "과목 변경후" from course;
--5) 만일 입력 실수로 student테이블의 sname컬럼에 데이터가 입력될 때
--   문자열 마지막에 공백이 추가되었다면 검색할 때 이를 제외하고
--   검색하는 SELECT 문을 작성하세요
select replace(sname,' ','') 이름, length(replace(sname,' ','')) 길이 from student;
