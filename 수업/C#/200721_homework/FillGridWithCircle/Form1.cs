...
namespace FillGridWithCircle {
    ...
    public partial class Form1 : Form {
        ...
        private void Form1_Paint(object sender, PaintEventArgs e) {
            ...
            // ellipse
            for(int i = 0; i < cnt; i++) {
                if(circleList[i].color == false) { g.DrawEllipse(new Pen(Color.Red,2), circleList[i].x, circleList[i].y, w, h);}
                else if(circleList[i].color == true) { g.DrawEllipse(new Pen(Color.Blue, 2), circleList[i].x, circleList[i].y, w, h);}
            }
        }
    }
}
